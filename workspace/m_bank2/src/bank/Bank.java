...
//	계좌번호 중복검사
	//모든 고객의 정보를 가져오기 위해 arrBank를 전달 받는다.
	//가입한 고객의 계좌번호만 가져오면 되기 때문에
	//이를 위해서는 은행별로 가입한 고객 수를 알아야 한다.
	//arCount는 은행별 고객수를 담고 있는 배열이다.
	//전달 받은 계좌번호(account)가 이미 존재하는 계좌 번호인지 검사한다.
	public final static Bank checkAccount(Bank[][] arrBank, int[] arCount, String account) {
//		전달 받은 계좌 번호 주인이 있다면, bank에 담아준다
...
//			안에있는 for문 밖에서도 j를 사용해야하기 때문에
//			이 영역에 선언한다.(중괄호를 벗어나면 사용하지 못함)
			int j = 0;
...
//			만약 위에 for문에서 j가 arCount[i]까지 증가하지 못했다면,
//			break를 만났다는 뜻이다. 즉 안에 있는 for문이 break로 종료되었다면,
//			밖에 있는 for문도 break로 탈출한다.
			if (j != arCount[i]) {
				break;
			}
		}
//		계좌번호를 찾았다면 그 계좌번호의 주인(객체)가 리턴되고,
//		계좌번호를 찾지 못한다면 null이 리턴된다.
		return bank;
	}
...
//		계좌번호가 존재한다면, 계좌번호 주인(객체)를 user에 담기고,
